...
Change these through the server's Configuration tab instead of the file editor, because hand edits to them get overwritten on the next start. Entries in `server.properties` that the panel doesn't manage are kept, and you can add your own through the **Custom Server Properties** field.
...
**Force Re-Provision** in the configuration tab reinstalls the server software and modpack on the next start without touching your world, your configs, or the mods you added. It's the fix for a broken install or a pack that won't update.
