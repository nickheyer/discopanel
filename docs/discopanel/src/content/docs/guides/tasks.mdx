...
- **On event** - the moment something happens: server starts, stops, restarts, becomes healthy, or a player joins or leaves.
...
- `X-DiscoPanel-Event` - the event name (`server_start`, `server_stop`, `server_restart`, `server_healthy`, `player_join`, `player_leave`, or `manual`)
