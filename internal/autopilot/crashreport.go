...
func parseReportMods(text string) []*agentv1.FailedMod {
	...
	if mods, _ := parseReportModSections(text); len(mods) > 0 {
		return mods
	}
	return parseFabricResolutionMods(text)
}

// Section forensics for one loader-blamed mod
type reportModDetail struct {
	FrameJars []string
	DistError bool
}

// Section forensics keyed by blamed mod id
func reportModDetails(dataPath, reportPath string) map[string]*reportModDetail {
	text := readCrashReport(dataPath, reportPath)
	if text == "" {
		return nil
	}
	_, details := parseReportModSections(text)
	return details
}

// Jar names inside report stack frame lines
var reportFrameJar = regexp.MustCompile(`~\[([^\[\]%!:]+\.jar)`)

// Client code markers that convict a dist error
func distErrorText(s string) bool {
	return strings.Contains(s, "net/minecraft/client/") ||
		strings.Contains(s, "net.minecraft.client.") ||
		strings.Contains(s, "invalid dist DEDICATED_SERVER")
}

// Reads 1.20 forge MOD sections, verdicts plus forensics
func parseReportModSections(text string) ([]*agentv1.FailedMod, map[string]*reportModDetail) {
	var mods []*agentv1.FailedMod
	details := map[string]*reportModDetail{}
	var cur *agentv1.FailedMod
	var det *reportModDetail
	for _, line := range strings.Split(text, "\n") {
		trimmed := strings.TrimSpace(line)
		if rest, ok := strings.CutPrefix(trimmed, "-- MOD "); ok {
		...
		if v, ok := strings.CutPrefix(trimmed, "Failure message:"); ok && cur.ErrorMessage == "" {
			cur.ErrorMessage = strings.TrimSpace(v)
		}
		if strings.HasPrefix(trimmed, "at ") {
			if m := reportFrameJar.FindStringSubmatch(trimmed); m != nil {
				det.FrameJars = append(det.FrameJars, m[1])
			}
			continue
		}
		if distErrorText(trimmed) {
			det.DistError = true
		}
	}
	return mods, details
