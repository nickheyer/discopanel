...
// idleState tracks how long a server has been without players.
type idleState struct {
	lastActive time.Time
	hadPlayers bool
}

// StartIdleWatcher launches the autopause/autostop policy loop.
...
// StopIdleWatcher stops the policy loop.
...
// checkIdleServers applies autopause/autostop policies to running servers.
func (m *Manager) checkIdleServers() {
	...
	for _, server := range servers {
		...
		cfg, err := m.store.GetServerConfig(ctx, server.ID)
		if err != nil {
			continue
		}
		...
		// Paused servers can still be autostopped after the stop timeout.
		...
		if autopause && idleFor >= m.timeoutFor(intOrDefault(cfg.AutopauseTimeoutEst, 3600), intOrDefault(cfg.AutopauseTimeoutInit, 600), hadPlayers) {
			if err := m.Pause(ctx, server.ID); err != nil {
...
// idleFor returns how long a server has been idle according to tracked state.
...
// idleTimeout resolves the applicable autostop timeout for a server.
func (m *Manager) idleTimeout(cfg *storage.ServerConfig, serverID string, _ bool) time.Duration {
	m.idleMu.Lock()
	hadPlayers := false
