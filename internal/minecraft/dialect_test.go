...
func TestResolveDialects(t *testing.T) {
	...
	if got := ResolveDialects(models.ModLoaderQuilt, dir, mods); !slices.Equal(got, []string{"quilt", "fabric"}) {
		t.Fatalf("quilt resolved %v", got)
	}
	if got := ResolveDialects(models.ModLoaderMohist, dir, mods); !slices.Equal(got, []string{"forge"}) {
		t.Fatalf("mohist resolved %v", got)
	}
	if got := ResolveDialects(models.ModLoaderPaper, dir, mods); got != nil {
		t.Fatalf("paper resolved %v, want none", got)
	}

	// Pack platforms declare nothing, the install testifies
	if got := ResolveDialects(models.ModLoaderModrinth, dir, mods); got != nil {
		t.Fatalf("empty install resolved %v", got)
	}
	if err := os.MkdirAll(filepath.Join(dir, "libraries", "net", "neoforged"), 0755); err != nil {
		t.Fatal(err)
	}
	if got := ResolveDialects(models.ModLoaderModrinth, dir, mods); !slices.Equal(got, []string{"neoforge", "forge"}) {
	...
	if got := ResolveDialects(models.ModLoaderModrinth, dir, mods); !slices.Equal(got, []string{"fabric"}) {
	...
	if got := ResolveDialects(models.ModLoaderCustom, dir, mods); !slices.Equal(got, []string{"forge"}) {
