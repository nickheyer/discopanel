<script lang="ts">
	...
	import type { Module, ModulePort, ModuleDependency, ModuleEventHook } from '$lib/proto/discopanel/v1/module_pb';
	import { ModuleStatus, ModuleEventType, ModuleEventAction } from '$lib/proto/discopanel/v1/module_pb';
	import { Loader2, Save, Package, Copy, Plus, Trash2, ChevronDown, ChevronUp } from '@lucide/svelte';
	...
	let { open = $bindable(), module, onUpdated }: Props = $props();

	let saving = $state(false);

	// Form state - initialized from module
	let name = $state('');
	let autoStart = $state(false);
	let followServerLifecycle = $state(false);
	...
	// Port helpers
	function addPort() {
		ports = [...ports, { name: '', containerPort: 0, hostPort: 0, protocol: 'tcp', proxyEnabled: true }];
	}
	...
	function parsePorts(modulePorts: ModulePort[] | undefined): PortConfig[] {
		if (!modulePorts || modulePorts.length === 0) return [];
	...
	function parseDependencies(deps: ModuleDependency[] | undefined): Dependency[] {
	...
	async function loadServerModules() {
		try {
			const response = await rpcClient.module.listModules({ serverId: module.serverId }, silentCallOptions);
			// Exclude current module from dependency options
			serverModules = response.modules.filter(m => m.id !== module.id);
		} catch {
			serverModules = [];
		}
	}
...
<Dialog bind:open>
	<DialogContent class="max-w-2xl max-h-[90vh] overflow-y-auto">
		<DialogHeader>
			...
			<DialogDescription>Update module configuration settings</DialogDescription>
		</DialogHeader>
		...
		<div class="flex items-center gap-4 py-2 px-1 text-xs text-muted-foreground border-b mb-2">
			...
			<AliasHelper serverId={module.serverId} moduleId={module.id} showLabel />
		</div>
		...
		<div class="space-y-6 py-4">
			...
			<div class="flex items-center gap-4 p-4 rounded-lg bg-muted/50">
				...
				<div class="flex-1">
					<div class="flex items-center gap-2">
						<span class="font-medium">{module.templateName}</span>
						<Badge variant="secondary" class="text-xs">{getStatusLabel(module.status)}</Badge>
					</div>
					<div class="flex items-center gap-2 mt-1">
						<span class="text-xs text-muted-foreground font-mono">{module.id}</span>
						<Button variant="ghost" size="icon" class="h-5 w-5" onclick={() => copyToClipboard(module.id)}>
							<Copy class="h-3 w-3" />
						</Button>
					</div>
					{#if module.containerId}
						<div class="flex items-center gap-2 mt-1">
							...
							<span class="text-xs font-mono text-muted-foreground truncate max-w-[200px]">{module.containerId}</span>
							<Button variant="ghost" size="icon" class="h-5 w-5" onclick={() => copyToClipboard(module.containerId)}>
								<Copy class="h-3 w-3" />
							</Button>
						</div>
					{/if}
			...
			<div class="border rounded-lg">
				...
				>
					<div>
						...
						<p class="text-xs text-muted-foreground">Dependencies, event hooks, and metadata</p>
					</div>
					{#if advancedExpanded}
			...
			{#if module.dataPath}
				<div class="space-y-2">
					<h4 class="text-sm font-medium">Data Location</h4>
					<div class="flex items-center gap-2 p-3 rounded-lg bg-muted/30">
						<span class="text-xs font-mono text-muted-foreground flex-1 truncate">{module.dataPath}</span>
						<Button variant="ghost" size="icon" class="h-6 w-6" onclick={() => copyToClipboard(module.dataPath)}>
							<Copy class="h-3 w-3" />
						</Button>
					</div>
				</div>
