<script lang="ts">
	...
	import ServerWebhooks from '$lib/components/server-webhooks.svelte';
	...
	<div class="h-full flex flex-col p-4 sm:p-6 lg:p-8 pt-4 sm:pt-6 bg-gradient-to-br from-background to-muted/20">
		...
		<Tabs value="overview" class="flex-1 flex flex-col min-h-0 gap-4" onValueChange={(value) => {
			...
			<div class="flex-shrink-0 w-full overflow-x-auto">
				<TabsList class="inline-flex w-full min-w-max sm:grid sm:grid-cols-9 h-12 sm:h-14 p-1 bg-muted/50 backdrop-blur-sm">
					...
					<TabsTrigger value="webhooks" class="data-[state=active]:bg-background data-[state=active]:shadow-lg data-[state=active]:text-foreground font-medium text-xs sm:text-sm whitespace-nowrap px-3 sm:px-4">Webhooks</TabsTrigger>
			...
			<div class="flex-1 min-h-0 overflow-hidden">
				...
				<TabsContent value="webhooks" class="h-full overflow-y-auto">
					<ServerWebhooks {server} active={activeTab === 'webhooks'} />
				</TabsContent>
